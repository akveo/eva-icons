  ...
  return Promise.all(config.convertOrResizeFormats.map((formatItem) => {
    sourceFiles.files.forEach((file, index) => {
      ...
      if (formatItem.size && formatItem.size.length !== 0) {
        ...
        const { format, ...resizeOptions } = convertAndResizeOptions;
        const writableStream = formatItem.format ? convertAndResize(convertAndResizeOptions) : resize(resizeOptions);
